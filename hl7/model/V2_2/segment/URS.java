...
public class URS extends hl7.model.V2_1.segment.URS{
	...
	static{
		...
		tables[3]=null;
		tables[4]=null;
		tables[5]=hl7.model.V2_2.table.Table0156.getInstance();
		tables[6]=hl7.model.V2_2.table.Table0157.getInstance();
		tables[7]=hl7.model.V2_2.table.Table0158.getInstance();
		
		descriptions[0]="R_U_WHERE_SUBJECT_DEFINITION";
		descriptions[1]="R_U_WHEN_DATA_START_DATE_TIME";
		descriptions[2]="R_U_WHEN_DATA_END_DATE_TIME";
		descriptions[3]="R_U_WHAT_USER_QUALIFIER";
		descriptions[4]="R_U_OTHER_RESULTS_SUBJECT_DEFINITION";
		descriptions[5]="R_U_WHICH_DATE_TIME_QUALIFIER";
		descriptions[6]="R_U_WHICH_DATE_TIME_STATUS_QUALIFIER";
		descriptions[7]="R_U_DATE_TIME_SELECTION_QUALIFIER";
	...
	public Segment cloneClass(String originalVersion, String setVersion) {
		hl7.pseudo.segment.URS seg = new hl7.pseudo.segment.URS();
	...
	public static void forward(DataStructure[][] components, DataStructure[][] subComponents, String originalVersion, String setVersion) throws Exception{ 
		for(int i=0; components!=null&&i<components.length; i++){
			...
			for(int c=0; c<comps.length; c++){
				...
				if(i==6) subComponents[i][c] = (components[i][c]!=null)?hl7.bean.datastructure.primitive.ID.CLASS.cloneClass(VERSION, VERSION):null;
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_1.segment.URS type = (hl7.model.V2_1.segment.URS)this;
	...
	public String encodeForward(String encodeVersion, String setVersion) throws Exception{
		...
		if(encodeVersion.equals(VERSION)){
			hl7.model.V2_3.segment.URS type = (hl7.model.V2_3.segment.URS)this;
