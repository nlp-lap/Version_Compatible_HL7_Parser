...
public class ADT_A07 extends hl7.model.V2_1.message.ADT_A07{
	...
	static{
		...
		standard[3]=hl7.pseudo.segment.MRG.CLASS;
		...
		standard[10]=hl7.pseudo.segment.PR1.CLASS;
		standard[11]=hl7.pseudo.segment.GT1.CLASS;
		standard[13]=hl7.pseudo.segment.ACC.CLASS;
		standard[14]=hl7.pseudo.segment.UB1.CLASS;
		standard[15]=hl7.pseudo.segment.UB2.CLASS;
		standard[12]=hl7.pseudo.group.INSURANCE_ADT_A07.CLASS;
	...
	public static void backward(Structure[][] components, Structure[][] subComponents, String originalVersion, String setVersion) throws Exception{
		...
		components[11] = subComponents[15];
		components[12] = subComponents[16];
		components[13] = subComponents[17];
		components[14] = subComponents[18];
		components[15] = subComponents[19];
		components[10] = null;
	}
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_1.message.ADT_A07 type = (hl7.model.V2_1.message.ADT_A07)this;
