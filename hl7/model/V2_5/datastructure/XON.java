...
public class XON extends hl7.model.V2_4.datastructure.XON{
	...
	static{
		...
		tables[0]=null;
		tables[1]=hl7.model.V2_5.table.Table0204.getInstance();
		tables[2]=null;
		tables[3]=null;
		tables[4]=hl7.model.V2_5.table.Table0061.getInstance();
		tables[5]=hl7.model.V2_5.table.Table0363.getInstance();
		tables[6]=hl7.model.V2_5.table.Table0203.getInstance();
		tables[7]=null;
		tables[8]=hl7.model.V2_5.table.Table0465.getInstance();
		tables[9]=null;
		
		descriptions[0]=ORGANIZATION_NAME;
		descriptions[1]=ORGANIZATION_NAME_TYPE_CODE;
		descriptions[2]=ID_NUMBER;
		descriptions[3]=CHECK_DIGIT;
	...
	public static void forward(DataStructure[] components, DataStructure[] subComponents, String originalVersion, String setVersion, int depth) throws Exception{ 
		for(int i=0; components!=null&&i<components.length; i++){
			if(components[i]==null) continue;
			if(i==6) subComponents[i] = (components[i]!=null)?hl7.bean.datastructure.primitive.ID.CLASS.cloneClass(VERSION, VERSION):null;
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_4.datastructure.XON type = (hl7.model.V2_4.datastructure.XON)this;
