...
public class XPN extends hl7.model.V2_4.datastructure.XPN{
	...
	static{
		...
		tables[3]=null;
		tables[4]=null;
		tables[5]=hl7.model.V2_5.table.Table0360.getInstance();
		tables[6]=hl7.model.V2_5.table.Table0200.getInstance();
		tables[7]=hl7.model.V2_5.table.Table0465.getInstance();
		tables[8]=hl7.model.V2_5.table.Table0448.getInstance();
		tables[9]=null;
		tables[10]=hl7.model.V2_5.table.Table0444.getInstance();
		tables[11]=null;
		tables[12]=null;
		tables[13]=null;
	...
	public static void forward(DataStructure[] components, DataStructure[] subComponents, String originalVersion, String setVersion, int depth) throws Exception{ 
		for(int i=0; components!=null&&i<components.length; i++){
			if(components[i]==null) continue;
			if(i==11) subComponents[i] = (components[i]!=null)?hl7.bean.datastructure.primitive.TS.CLASS.cloneClass(VERSION, VERSION):null;
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_4.datastructure.XPN type = (hl7.model.V2_4.datastructure.XPN)this;
