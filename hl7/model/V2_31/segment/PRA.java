...
public class PRA extends hl7.model.V2_3.segment.PRA{
	...
	static{
		...
		tables[0]=null;
		tables[1]=hl7.model.V2_31.table.Table0358.getInstance();
		tables[2]=hl7.model.V2_31.table.Table0186.getInstance();
		tables[3]=hl7.model.V2_31.table.Table0187.getInstance();
		tables[4]=hl7.model.V2_31.table.Table0337.getInstance();
		tables[5]=hl7.model.V2_31.table.Table0338.getInstance();
		tables[6]=null;
		tables[7]=null;
	...
	public static void forward(DataStructure[][] components, DataStructure[][] subComponents, String originalVersion, String setVersion) throws Exception{ 
		for(int i=0; components!=null&&i<components.length; i++){
			...
			for(int c=0; c<comps.length; c++){
				...
				if(i==4) subComponents[i][c] = (components[i][c]!=null)?hl7.pseudo.datastructure.SPD.CLASS.cloneClass(VERSION, VERSION):null;
				if(i==5) subComponents[i][c] = (components[i][c]!=null)?hl7.pseudo.datastructure.PLN.CLASS.cloneClass(VERSION, VERSION):null;
				if(i==6) subComponents[i][c] = (components[i][c]!=null)?hl7.pseudo.datastructure.PIP.CLASS.cloneClass(VERSION, VERSION):null;
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_3.segment.PRA type = (hl7.model.V2_3.segment.PRA)this;
	...
	public String encodeForward(String encodeVersion, String setVersion) throws Exception{
		...
		if(encodeVersion.equals(VERSION)){
			hl7.model.V2_4.segment.PRA type = (hl7.model.V2_4.segment.PRA)this;
