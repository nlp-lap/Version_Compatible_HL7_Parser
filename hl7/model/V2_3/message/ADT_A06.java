...
public class ADT_A06 extends hl7.model.V2_2.message.ADT_A06{
	...
	static{
		...
		standard[14]=hl7.pseudo.group.PROCEDURE_ADT_A06.CLASS;
		standard[16]=hl7.pseudo.group.INSURANCE_ADT_A06.CLASS;
	...
	public static void backward(Structure[][] components, Structure[][] subComponents, String originalVersion, String setVersion) throws Exception{
		...
		components[14] = subComponents[13];
		components[15] = subComponents[14];
		components[16] = subComponents[15];
		components[17] = subComponents[16];
		components[18] = subComponents[17];
		components[19] = subComponents[18];
		components[10] = null;
		components[11] = null;
		components[12] = null;
		components[13] = null;
	...
	public static void forward(Structure[][] components, Structure[][] subComponents, String originalVersion, String setVersion) throws Exception{ 
		...
		subComponents[15] = components[11];
		subComponents[16] = components[12];
		subComponents[17] = components[13];
		subComponents[18] = components[14];
		subComponents[19] = components[15];
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_2.message.ADT_A06 type = (hl7.model.V2_2.message.ADT_A06)this;
