...
public class OM2 extends hl7.model.V2_2.segment.OM2{
	...
	static{
		...
		standard[6]=hl7.pseudo.datastructure.CM.CLASS;
		standard[7]=hl7.pseudo.datastructure.CM.CLASS;
		standard[8]=hl7.pseudo.datastructure.CM.CLASS;
		...
		descriptions[1]="UNITS_OF_MEASURE";
		descriptions[2]="RANGE_OF_DECIMAL_PRECISION";
		descriptions[3]="CORRESPONDING_SI_UNITS_OF_MEASURE";
		descriptions[4]="SI_CONVERSION_FACTOR";
		descriptions[5]="REFERENCE_RANGE_ORDINAL_CONTINUOUS_OBS";
		descriptions[6]="CRITICAL_RANGE_FOR_ORDINAL_CONTINUOUS_OBS";
		descriptions[7]="ABSOLUTE_RANGE_FOR_ORDINAL_CONTINUOUS_OBS";
		descriptions[8]="DELTA_CHECK_CRITERIA";
		descriptions[9]="MINIMUM_MEANINGFUL_INCREMENTS";
	...
	public Segment cloneClass(String originalVersion, String setVersion) {
		hl7.pseudo.segment.OM2 seg = new hl7.pseudo.segment.OM2();
	...
	public static void backward(DataStructure[][] components, DataStructure[][] subComponents, String originalVersion, String setVersion, String prevVersion) throws Exception{
		for(int i=0; subComponents!=null&&i<subComponents.length; i++){
			...
			for(int c=0; c<subComps.length; c++){
				...
				if(i==6) components[i][c] = (subComponents[i][c]!=null)?hl7.pseudo.datastructure.CM.CLASS.cloneClass(prevVersion, prevVersion):null;
				if(i==7) components[i][c] = (subComponents[i][c]!=null)?hl7.pseudo.datastructure.CM.CLASS.cloneClass(prevVersion, prevVersion):null;
				if(i==8) components[i][c] = (subComponents[i][c]!=null)?hl7.pseudo.datastructure.CM.CLASS.cloneClass(prevVersion, prevVersion):null;
	...
	public static void forward(DataStructure[][] components, DataStructure[][] subComponents, String originalVersion, String setVersion) throws Exception{ 
		for(int i=0; components!=null&&i<components.length; i++){
			...
			for(int c=0; c<comps.length; c++){
				...
				if(i==2) subComponents[i][c] = (components[i][c]!=null)?hl7.bean.datastructure.primitive.NM.CLASS.cloneClass(VERSION, VERSION):null;
				if(i==4) subComponents[i][c] = (components[i][c]!=null)?hl7.bean.datastructure.primitive.TX.CLASS.cloneClass(VERSION, VERSION):null;
				...
				if(i==3) subComponents[i][c] = (components[i][c]!=null)?hl7.pseudo.datastructure.CE.CLASS.cloneClass(VERSION, VERSION):null;
				if(i==5) subComponents[i][c] = (components[i][c]!=null)?hl7.pseudo.datastructure.CM.CLASS.cloneClass(VERSION, VERSION):null;
				if(i==6) subComponents[i][c] = (components[i][c]!=null)?hl7.pseudo.datastructure.CM.CLASS.cloneClass(VERSION, VERSION):null;
				if(i==7) subComponents[i][c] = (components[i][c]!=null)?hl7.pseudo.datastructure.CM.CLASS.cloneClass(VERSION, VERSION):null;
				if(i==8) subComponents[i][c] = (components[i][c]!=null)?hl7.pseudo.datastructure.CM.CLASS.cloneClass(VERSION, VERSION):null;
				if(components[i][c]==null) continue;
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_2.segment.OM2 type = (hl7.model.V2_2.segment.OM2)this;
	...
	public String encodeForward(String encodeVersion, String setVersion) throws Exception{
		...
		if(encodeVersion.equals(VERSION)){
			hl7.model.V2_31.segment.OM2 type = (hl7.model.V2_31.segment.OM2)this;
