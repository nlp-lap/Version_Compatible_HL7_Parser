...
public class ORC extends hl7.model.V2_2.segment.ORC{
	...
	static{
		...
		tables[0]=hl7.model.V2_3.table.Table0119.getInstance();
		tables[1]=null;
		tables[2]=null;
		tables[3]=null;
		tables[4]=hl7.model.V2_3.table.Table0038.getInstance();
		tables[5]=hl7.model.V2_3.table.Table0121.getInstance();
		tables[6]=null;
		tables[7]=null;
		tables[8]=null;
		tables[9]=null;
	...
	public static void forward(DataStructure[][] components, DataStructure[][] subComponents, String originalVersion, String setVersion) throws Exception{ 
		for(int i=0; components!=null&&i<components.length; i++){
			...
			for(int c=0; c<comps.length; c++){
				...
				if(i==3) subComponents[i][c] = (components[i][c]!=null)?hl7.pseudo.datastructure.EI.CLASS.cloneClass(VERSION, VERSION):null;
				...
				if(i==12) subComponents[i][c] = (components[i][c]!=null)?hl7.pseudo.datastructure.PL.CLASS.cloneClass(VERSION, VERSION):null;
				if(i==18) subComponents[i][c] = (components[i][c]!=null)?hl7.pseudo.datastructure.XCN.CLASS.cloneClass(VERSION, VERSION):null;
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_2.segment.ORC type = (hl7.model.V2_2.segment.ORC)this;
	...
	public String encodeForward(String encodeVersion, String setVersion) throws Exception{
		...
		if(encodeVersion.equals(VERSION)){
			hl7.model.V2_31.segment.ORC type = (hl7.model.V2_31.segment.ORC)this;
