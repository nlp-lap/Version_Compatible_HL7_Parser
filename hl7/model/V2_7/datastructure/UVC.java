...
public class UVC extends hl7.model.V2_6.datastructure.UVC{
	public static final String VERSION = "2.7";
	public static final String VALUE_CODE = "Value Code";
	public static final String VALUE_AMOUNT = "Value Amount";
	public static final String NON_MONETARY_VALUE_AMOUNT_QUANTITY = "Non-Monetary Value Amount / Quantity";
	public static final String NON_MONETARY_VALUE_AMOUNT_UNITS = "Non-Monetary Value Amount / Units";
	...
	static{
		...
		standard[1]=hl7.pseudo.datastructure.MO.CLASS;
		standard[2]=hl7.bean.datastructure.primitive.NM.CLASS;
		standard[3]=hl7.pseudo.datastructure.CWE.CLASS;
		
		tables[0]=hl7.model.V2_7.table.Table0153.getInstance();
		tables[1]=null;
		tables[2]=null;
		tables[3]=null;
		
		descriptions[0]=VALUE_CODE;
		descriptions[1]=VALUE_AMOUNT;
		descriptions[2]=NON_MONETARY_VALUE_AMOUNT_QUANTITY;
		descriptions[3]=NON_MONETARY_VALUE_AMOUNT_UNITS;
	...
	public UVC(){
		type = DataStructure.UVC;
	}
	
	@Override
	public DataStructure cloneClass(String originalVersion, String setVersion) {
		hl7.pseudo.datastructure.UVC data = new hl7.pseudo.datastructure.UVC();
	...
	public static void forward(DataStructure[] components, DataStructure[] subComponents, String originalVersion, String setVersion, int depth) throws Exception{ 
		for(int i=0; components!=null&&i<components.length; i++){
			if(components[i]==null) continue;
			if(i==2) subComponents[i] = (components[i]!=null)?hl7.bean.datastructure.primitive.NM.CLASS.cloneClass(VERSION, VERSION):null;
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_6.datastructure.UVC type = (hl7.model.V2_6.datastructure.UVC)this;
