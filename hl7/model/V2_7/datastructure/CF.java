...
public class CF extends hl7.model.V2_6.datastructure.CF{
	...
	public static final String SECOND_ALTERNATE_FORMATTED_TEXT = "Second Alternate Formatted Text";
	...
	static{
		...
		standard[10]=hl7.bean.datastructure.primitive.FT.CLASS;
		...
		descriptions[10]=SECOND_ALTERNATE_FORMATTED_TEXT;
	...
	public static void forward(DataStructure[] components, DataStructure[] subComponents, String originalVersion, String setVersion, int depth) throws Exception{ 
		for(int i=0; components!=null&&i<components.length; i++){
			if(components[i]==null) continue;
			if(i==6) subComponents[i] = (components[i]!=null)?hl7.bean.datastructure.primitive.ST.CLASS.cloneClass(VERSION, VERSION):null;
			...
			if(i==10) subComponents[i] = (components[i]!=null)?hl7.bean.datastructure.primitive.FT.CLASS.cloneClass(VERSION, VERSION):null;
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_6.datastructure.CF type = (hl7.model.V2_6.datastructure.CF)this;
