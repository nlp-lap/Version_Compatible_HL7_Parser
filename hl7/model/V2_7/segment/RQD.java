...
public class RQD extends hl7.model.V2_6.segment.RQD{
	...
	static{
		...
		standard[6]=hl7.pseudo.datastructure.CX.CLASS;
		...
		tables[6]=hl7.model.V2_7.table.Table0319.getInstance();
		tables[7]=hl7.model.V2_7.table.Table0320.getInstance();
		tables[8]=hl7.model.V2_7.table.Table9999.getInstance();
		tables[9]=null;
		
		descriptions[0]="REQUISITION_LINE_NUMBER";
		descriptions[1]="ITEM_CODE_INTERNAL";
		descriptions[2]="ITEM_CODE_EXTERNAL";
		descriptions[3]="HOSPITAL_ITEM_CODE";
		descriptions[4]="REQUISITION_QUANTITY";
		descriptions[5]="REQUISITION_UNIT_OF_MEASURE";
		descriptions[6]="COST_CENTER_ACCOUNT_NUMBER";
		descriptions[7]="ITEM_NATURAL_ACCOUNT_CODE";
		descriptions[8]="DELIVER_TO_ID";
		descriptions[9]="DATE_NEEDED";
	...
	public Segment cloneClass(String originalVersion, String setVersion) {
		hl7.pseudo.segment.RQD seg = new hl7.pseudo.segment.RQD();
	...
	public static void forward(DataStructure[][] components, DataStructure[][] subComponents, String originalVersion, String setVersion) throws Exception{ 
		for(int i=0; components!=null&&i<components.length; i++){
			...
			for(int c=0; c<comps.length; c++){
				if(i==6) subComponents[i][c] = (components[i][c]!=null)?hl7.pseudo.datastructure.CX.CLASS.cloneClass(VERSION, VERSION):null;
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_6.segment.RQD type = (hl7.model.V2_6.segment.RQD)this;
