...
public class PKG extends hl7.model.V2_6.segment.PKG{
	...
	static{
		...
		tables[0]=null;
		tables[1]=hl7.model.V2_7.table.Table0818.getInstance();
		tables[2]=hl7.model.V2_7.table.Table0532.getInstance();
		tables[3]=null;
		tables[4]=null;
		tables[5]=null;
		tables[6]=null;
		
		descriptions[0]="SET_ID_PKG";
		descriptions[1]="PACKAGING_UNITS";
		descriptions[2]="DEFAULT_ORDER_UNIT_OF_MEASURE_INDICATOR";
		descriptions[3]="PACKAGE_QUANTITY";
		descriptions[4]="PRICE";
		descriptions[5]="FUTURE_ITEM_PRICE";
		descriptions[6]="FUTURE_ITEM_PRICE_EFFECTIVE_DATE";
	...
	public Segment cloneClass(String originalVersion, String setVersion) {
		hl7.pseudo.segment.PKG seg = new hl7.pseudo.segment.PKG();
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_6.segment.PKG type = (hl7.model.V2_6.segment.PKG)this;
