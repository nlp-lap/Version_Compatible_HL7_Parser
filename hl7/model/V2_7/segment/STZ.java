...
public class STZ extends hl7.model.V2_6.segment.STZ{
	...
	static{
		...
		tables[0]=hl7.model.V2_7.table.Table0806.getInstance();
		tables[1]=hl7.model.V2_7.table.Table0702.getInstance();
		tables[2]=hl7.model.V2_7.table.Table0809.getInstance();
		tables[3]=hl7.model.V2_7.table.Table0811.getInstance();
		
		descriptions[0]="STERILIZATION_TYPE";
		descriptions[1]="STERILIZATION_CYCLE";
		descriptions[2]="MAINTENANCE_CYCLE";
		descriptions[3]="MAINTENANCE_TYPE";
	...
	public Segment cloneClass(String originalVersion, String setVersion) {
		hl7.pseudo.segment.STZ seg = new hl7.pseudo.segment.STZ();
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_6.segment.STZ type = (hl7.model.V2_6.segment.STZ)this;
