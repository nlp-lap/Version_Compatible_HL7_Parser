...
public class PCE extends hl7.model.V2_6.segment.PCE{
	...
	static{
		...
		standard[3]=hl7.pseudo.datastructure.CP.CLASS;
		
		tables[0]=null;
		tables[1]=hl7.model.V2_7.table.Table0319.getInstance();
		tables[2]=hl7.model.V2_7.table.Table0132.getInstance();
		tables[3]=null;
		
		descriptions[0]="SET_ID_PCE";
		descriptions[1]="COST_CENTER_ACCOUNT_NUMBER";
		descriptions[2]="TRANSACTION_CODE";
		descriptions[3]="TRANSACTION_AMOUNT_UNIT";
	...
	public Segment cloneClass(String originalVersion, String setVersion) {
		hl7.pseudo.segment.PCE seg = new hl7.pseudo.segment.PCE();
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_6.segment.PCE type = (hl7.model.V2_6.segment.PCE)this;
