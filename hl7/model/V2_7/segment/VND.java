...
public class VND extends hl7.model.V2_6.segment.VND{
	...
	static{
		...
		standard[4]=hl7.pseudo.datastructure.CNE.CLASS;
		
		tables[0]=null;
		tables[1]=null;
		tables[2]=null;
		tables[3]=null;
		tables[4]=hl7.model.V2_7.table.Table0532.getInstance();
		
		descriptions[0]="SET_ID_VND";
		descriptions[1]="VENDOR_IDENTIFIER";
		descriptions[2]="VENDOR_NAME";
		descriptions[3]="VENDOR_CATALOG_NUMBER";
		descriptions[4]="PRIMARY_VENDOR_INDICATOR";
	...
	public Segment cloneClass(String originalVersion, String setVersion) {
		hl7.pseudo.segment.VND seg = new hl7.pseudo.segment.VND();
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_6.segment.VND type = (hl7.model.V2_6.segment.VND)this;
