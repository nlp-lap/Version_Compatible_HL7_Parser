...
public class DMI extends hl7.model.V2_6.segment.DMI{
	...
	static{
		...
		tables[1]=hl7.model.V2_7.table.Table0118.getInstance();
		tables[2]=null;
		tables[3]=null;
		tables[4]=null;
		
		descriptions[0]="DIAGNOSTIC_RELATED_GROUP";
		descriptions[1]="MAJOR_DIAGNOSTIC_CATEGORY";
		descriptions[2]="LOWER_AND_UPPER_TRIM_POINTS";
		descriptions[3]="AVERAGE_LENGTH_OF_STAY";
		descriptions[4]="RELATIVE_WEIGHT";
	...
	public Segment cloneClass(String originalVersion, String setVersion) {
		hl7.pseudo.segment.DMI seg = new hl7.pseudo.segment.DMI();
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_6.segment.DMI type = (hl7.model.V2_6.segment.DMI)this;
