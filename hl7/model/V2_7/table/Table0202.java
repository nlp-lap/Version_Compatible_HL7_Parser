package hl7.model.V2_7.table;

import hl7.bean.table.Table;

public class Table0202 extends Table{
	private static final String VERSION = "2.7";
	public static Table getInstance(){
		if(table==null) new Table0202();
		return table;
	}

	public static final String BP = "BP";
	public static final String CP = "CP";
	public static final String FX = "FX";
	public static final String Internet = "Internet";
	public static final String MD = "MD";
	public static final String PH = "PH";
	public static final String SAT = "SAT";
	public static final String TDD = "TDD";
	public static final String TTY = "TTY";
	public static final String X_400 = "X.400";

	private Table0202(){
		setTableName("Telecommunication Equipment Type");
		setOID("");

		putMap("BP", "Beeper");
		putMap("CP", "Cellular or Mobile Phone");
		putMap("FX", "Fax");
		putMap("Internet", "Internet Address");
		putMap("MD", "Modem");
		putMap("PH", "Telephone");
		putMap("SAT", "Satellite Phone");
		putMap("TDD", "Telecommunications Device for the Deaf");
		putMap("TTY", "Teletypewriter");
		putMap("X.400", "X.400 email address");

	}

}
