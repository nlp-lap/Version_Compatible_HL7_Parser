...
public class SPS extends hl7.model.V2_31.datastructure.SPS{
	...
	public static final String COLLECTION_MODIFIER_METHOD_CODE = "collection modifier method code";
	...
	static{
		...
		standard[1]=hl7.bean.datastructure.primitive.TX.CLASS;
		...
		descriptions[2]=FREETEXT;
		descriptions[3]=BODY_SITE;
		descriptions[4]=SITE_MODIFIER;
		descriptions[5]=COLLECTION_MODIFIER_METHOD_CODE;
	...
	public static void backward(DataStructure[] components, DataStructure[] subComponents, String originalVersion, String setVersion, String prevVersion, int depth) throws Exception{
		for(int i=0; subComponents!=null&&i<subComponents.length; i++){
			...
			if(i==1) components[i] = (subComponents[i]!=null)?hl7.bean.datastructure.primitive.TX.CLASS.cloneClass(prevVersion, prevVersion):null;
			if(i==0) components[i] = (subComponents[i]!=null)?hl7.pseudo.datastructure.CE.CLASS.cloneClass(prevVersion, prevVersion):null;
			if(i==3) components[i] = (subComponents[i]!=null)?hl7.pseudo.datastructure.CE.CLASS.cloneClass(prevVersion, prevVersion):null;
			if(i==4) components[i] = (subComponents[i]!=null)?hl7.pseudo.datastructure.CE.CLASS.cloneClass(prevVersion, prevVersion):null;
			if(i==5) components[i] = (subComponents[i]!=null)?hl7.pseudo.datastructure.CE.CLASS.cloneClass(prevVersion, prevVersion):null;
			if(i==6) components[i] = (subComponents[i]!=null)?hl7.pseudo.datastructure.CE.CLASS.cloneClass(prevVersion, prevVersion):null;
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_31.datastructure.SPS type = (hl7.model.V2_31.datastructure.SPS)this;
	...
	public String encodeForward(String encodeVersion, String setVersion) throws Exception{
		...
		if(encodeVersion.equals(VERSION)){
			hl7.model.V2_5.datastructure.SPS type = (hl7.model.V2_5.datastructure.SPS)this;
