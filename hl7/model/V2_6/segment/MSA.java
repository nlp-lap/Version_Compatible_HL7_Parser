...
public class MSA extends hl7.model.V2_51.segment.MSA{
	...
	static{
		...
		tables[0]=hl7.model.V2_6.table.Table0008.getInstance();
		tables[1]=null;
		tables[2]=null;
		tables[3]=null;
		tables[4]=null;
		tables[5]=hl7.model.V2_6.table.Table0357.getInstance();
		tables[6]=null;
		tables[7]=hl7.model.V2_6.table.Table0520.getInstance();
		
		descriptions[0]="ACKNOWLEDGMENT_CODE";
		descriptions[1]="MESSAGE_CONTROL_ID";
		descriptions[2]="TEXT_MESSAGE";
		descriptions[3]="EXPECTED_SEQUENCE_NUMBER";
		descriptions[4]="DELAYED_ACKNOWLEDGMENT_TYPE";
		descriptions[5]="ERROR_CONDITION";
		descriptions[6]="MESSAGE_WAITING_NUMBER";
		descriptions[7]="MESSAGE_WAITING_PRIORITY";
	...
	public Segment cloneClass(String originalVersion, String setVersion) {
		hl7.pseudo.segment.MSA seg = new hl7.pseudo.segment.MSA();
	...
	public static void forward(DataStructure[][] components, DataStructure[][] subComponents, String originalVersion, String setVersion) throws Exception{ 
		for(int i=0; components!=null&&i<components.length; i++){
			...
			for(int c=0; c<comps.length; c++){
				...
				if(i==6) subComponents[i][c] = (components[i][c]!=null)?hl7.bean.datastructure.primitive.NM.CLASS.cloneClass(VERSION, VERSION):null;
				if(i==7) subComponents[i][c] = (components[i][c]!=null)?hl7.bean.datastructure.primitive.ID.CLASS.cloneClass(VERSION, VERSION):null;
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_51.segment.MSA type = (hl7.model.V2_51.segment.MSA)this;
	...
	public String encodeForward(String encodeVersion, String setVersion) throws Exception{
		...
		if(encodeVersion.equals(VERSION)){
			hl7.model.V2_7.segment.MSA type = (hl7.model.V2_7.segment.MSA)this;
