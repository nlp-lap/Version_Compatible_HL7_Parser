...
public class TQ extends hl7.model.V2_51.datastructure.TQ{
	...
	static{
		...
		tables[6]=null;
		tables[7]=null;
		tables[8]=hl7.model.V2_6.table.Table0472.getInstance();
		tables[9]=null;
		tables[10]=null;
		tables[11]=null;
	...
	public static void forward(DataStructure[] components, DataStructure[] subComponents, String originalVersion, String setVersion, int depth) throws Exception{ 
		for(int i=0; components!=null&&i<components.length; i++){
			if(components[i]==null) continue;
			if(i==3) subComponents[i] = (components[i]!=null)?hl7.bean.datastructure.primitive.DTM.CLASS.cloneClass(VERSION, VERSION):null;
			if(i==4) subComponents[i] = (components[i]!=null)?hl7.bean.datastructure.primitive.DTM.CLASS.cloneClass(VERSION, VERSION):null;
			if(i==10) subComponents[i] = (components[i]!=null)?hl7.pseudo.datastructure.CWE.CLASS.cloneClass(VERSION, VERSION):null;
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_51.datastructure.TQ type = (hl7.model.V2_51.datastructure.TQ)this;
