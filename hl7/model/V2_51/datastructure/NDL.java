...
public class NDL extends hl7.model.V2_5.datastructure.NDL{
	...
	static{
		standard[0]=hl7.pseudo.datastructure.CNN.CLASS;
		...
		tables[1]=null;
		tables[2]=null;
		tables[3]=hl7.model.V2_51.table.Table0302.getInstance();
		tables[4]=hl7.model.V2_51.table.Table0303.getInstance();
		tables[5]=hl7.model.V2_51.table.Table0304.getInstance();
		tables[6]=null;
		tables[7]=hl7.model.V2_51.table.Table0306.getInstance();
		tables[8]=hl7.model.V2_51.table.Table0305.getInstance();
		tables[9]=hl7.model.V2_51.table.Table0307.getInstance();
		tables[10]=hl7.model.V2_51.table.Table0308.getInstance();
		
		descriptions[0]=NAME;
		descriptions[1]=START_DATE_TIME;
		descriptions[2]=END_DATE_TIME;
		descriptions[3]=POINT_OF_CARE;
		descriptions[4]=ROOM;
		descriptions[5]=BED;
		descriptions[6]=FACILITY;
		descriptions[7]=LOCATION_STATUS;
		descriptions[8]=PATIENT_LOCATION_TYPE;
		descriptions[9]=BUILDING;
		descriptions[10]=FLOOR;
	...
	public NDL(){
		type = DataStructure.NDL;
	}
	
	@Override
	public DataStructure cloneClass(String originalVersion, String setVersion) {
		hl7.pseudo.datastructure.NDL data = new hl7.pseudo.datastructure.NDL();
	...
	public static void backward(DataStructure[] components, DataStructure[] subComponents, String originalVersion, String setVersion, String prevVersion, int depth) throws Exception{
		for(int i=0; subComponents!=null&&i<subComponents.length; i++){
			...
			if(i==1) components[i] = (subComponents[i]!=null)?hl7.bean.datastructure.primitive.TS.CLASS.cloneClass(prevVersion, prevVersion):null;
			if(i==2) components[i] = (subComponents[i]!=null)?hl7.bean.datastructure.primitive.TS.CLASS.cloneClass(prevVersion, prevVersion):null;
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_5.datastructure.NDL type = (hl7.model.V2_5.datastructure.NDL)this;
	...
	public String encodeForward(String encodeVersion, String setVersion) throws Exception{
		...
		if(encodeVersion.equals(VERSION)){
			hl7.model.V2_6.datastructure.NDL type = (hl7.model.V2_6.datastructure.NDL)this;
