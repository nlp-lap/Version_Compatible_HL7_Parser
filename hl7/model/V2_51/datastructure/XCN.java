...
public class XCN extends hl7.model.V2_5.datastructure.XCN{
	...
	static{
		...
		tables[13]=null;
		tables[14]=hl7.model.V2_51.table.Table0465.getInstance();
		tables[15]=hl7.model.V2_51.table.Table0448.getInstance();
		tables[16]=null;
		tables[17]=hl7.model.V2_51.table.Table0444.getInstance();
		tables[18]=null;
		tables[19]=null;
		tables[20]=null;
		tables[21]=null;
	...
	public static void backward(DataStructure[] components, DataStructure[] subComponents, String originalVersion, String setVersion, String prevVersion, int depth) throws Exception{
		for(int i=0; subComponents!=null&&i<subComponents.length; i++){
			...
			if(i==18) components[i] = (subComponents[i]!=null)?hl7.bean.datastructure.primitive.TS.CLASS.cloneClass(prevVersion, prevVersion):null;
			...
			if(i==15) components[i] = (subComponents[i]!=null)?hl7.pseudo.datastructure.CE.CLASS.cloneClass(prevVersion, prevVersion):null;
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_5.datastructure.XCN type = (hl7.model.V2_5.datastructure.XCN)this;
