...
public class XAD extends hl7.model.V2_5.datastructure.XAD{
	...
	static{
		...
		tables[3]=null;
		tables[4]=null;
		tables[5]=hl7.model.V2_51.table.Table0399.getInstance();
		tables[6]=hl7.model.V2_51.table.Table0190.getInstance();
		tables[7]=null;
		tables[8]=hl7.model.V2_51.table.Table0289.getInstance();
		tables[9]=hl7.model.V2_51.table.Table0288.getInstance();
		tables[10]=hl7.model.V2_51.table.Table0465.getInstance();
		tables[11]=null;
		tables[12]=null;
		tables[13]=null;
	...
	public static void backward(DataStructure[] components, DataStructure[] subComponents, String originalVersion, String setVersion, String prevVersion, int depth) throws Exception{
		for(int i=0; subComponents!=null&&i<subComponents.length; i++){
			...
			if(i==13) components[i] = (subComponents[i]!=null)?hl7.bean.datastructure.primitive.TS.CLASS.cloneClass(prevVersion, prevVersion):null;
		...
		if(seekOriginalVersion&&MessageStructure.getVersionCode(originalVersion)<MessageStructure.getVersionCode(VERSION)){ //실제 버전의 위치 찾기
			...
			if(setVersion.equals(VERSION)){ //설정 버전의 위치 찾기
				...
				if(MessageStructure.getVersionCode(setVersion)<MessageStructure.getVersionCode(VERSION)){ //버전으로 이동 방향 찾기
					...
					//if(MessageStructure.getVersionCode(originalVersion)>=MessageStructure.getVersionCode(VERSION)){
						hl7.model.V2_5.datastructure.XAD type = (hl7.model.V2_5.datastructure.XAD)this;
